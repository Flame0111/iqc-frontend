...
export default function DatabaseListPage({ onAddNew, onEditRecord }) {
  ...
  // 📥 ตัวอย่างการ Fetch ข้อมูล (จิมมี่เปลี่ยน URL API ได้เลย)
  useEffect(() => {
    // สมมติว่าดึงข้อมูลจาก API ของจิมมี่
    setTimeout(() => {
      setRecords([
        { id: 1, name: 'Data 001', detail: 'Description A', status: 'Active', date: '2026-06-15' },
        { id: 2, name: 'Data 002', detail: 'Description B', status: 'Pending', date: '2026-06-16' }
      ]);
      setLoading(false);
    }, 1000);
  }, []);
  ...
  const getStatusBadge = (status) => {
    ...
    return <span className="px-2.5 py-1 text-[11px] font-bold rounded-lg bg-emerald-500/10 text-emerald-400 border border-emerald-500/20">ACTIVE</span>;
  };
  ...
  return (
    ...
    >
      {/* ส่วนหัวของหน้าจอ & ปุ่มสร้างข้อมูลใหม่ */}
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 border-b border-white/5 pb-6">
        <div>
          <h1 className="text-xl font-black uppercase text-white tracking-wider flex items-center gap-2">
            <Database className="text-[#6f7bf7]" /> Main Database
          </h1>
          <p className="text-xs text-white/40 mt-1">จัดการและตรวจสอบรายการข้อมูลทั้งหมดในระบบ</p>
        </div>

        <motion.button
          whileHover={{ scale: 1.03, boxShadow: "0 0 30px rgba(111,123,247,0.3)" }}
          whileTap={{ scale: 0.97 }}
          onClick={onAddNew}
          className="bg-white text-black font-black text-sm py-3 px-6 rounded-2xl flex items-center gap-2 shadow-lg cursor-pointer"
        >
      ...
      <div className="relative w-full max-w-md">
        ...
        <input
          ...
          className="w-full bg-white/5 border border-white/10 rounded-2xl pl-11 pr-4 py-3 text-sm font-bold text-white placeholder-white/20 focus:outline-none focus:border-[#6f7bf7]/50 transition-all shadow-inner"
        />
      </div>
          ...
          <table className="w-full text-sm text-left border-collapse">
            <thead>
              <tr className="bg-white/[0.02] border-b border-white/10 text-[10px] font-bold text-white/40 uppercase tracking-wider">
                <th className="py-4 px-6">Record Name</th>
                <th className="py-4 px-4">Detail</th>
              ...
              {filteredRecords.length === 0 ? (
                ...
                filteredRecords.map((row) => (
                  <tr key={row.id} className="hover:bg-white/[0.02] transition-colors group text-white/80">
                    ...
                    <td className="py-4 px-4 text-xs font-bold text-white/60">{row.detail}</td>
                    ...
                    <td className="py-4 px-6 text-right align-middle">
                      <motion.button
                        ...
                        className="p-2 bg-white/5 border border-white/10 hover:border-[#6f7bf7] hover:bg-[#6f7bf7]/10 hover:text-[#6f7bf7] rounded-xl text-white/60 transition-all inline-flex items-center gap-1.5 text-xs font-bold cursor-pointer"
                      >
                        <Edit2 size={13} /> EDIT
                      </motion.button>
                    </td>
